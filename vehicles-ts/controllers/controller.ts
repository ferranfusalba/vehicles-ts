"use strict";
var newCar: HTMLFormElement = document.forms[0];
var newWheels: HTMLFormElement = document.forms[1];
var displayInfo = document.getElementById('displayInfo');
var matricula: any = document.getElementById("plate");
var marca: any = document.getElementById("brand");
var color: any = document.getElementById("color");
var color2: any = document.getElementById("color");
var car:Car;
var cars:Car[] = [];
...
function createCar(plate:string, brand:string, color:string) {
    ...
    if (!validPlate(matricula.value)) {
        showError(matricula, "errorMatricula", "La matrícula ha de ser de 4 xifres i 3 lletres seguides en majúscula. Exemple: 2134XYZ");
        error++;
    ...
    if (!validBrand(marca.value)) {
        showError(marca, "errorBrand", "Aquest camp és obligatori.");
        error++;
    ...
    if (!validColor(color2.value)) {
        showError(color2, "errorColor", "Aquest camp és obligatori.");
        error++;
...
function createWheels(w1M:string, w1D:number, w2M:string, w2D:number, w3M:string, w3D:number, w4M:string, w4D:number) {
    ...
    if (!validBrand(w1M)) {
        showError(document.getElementById("w1M"), "errorw1M", "Aquest camp és obligatori.");
        error++;
    ...
    if (!validBrand(w2M)) {
        showError(document.getElementById("w2M"), "errorw2M", "Aquest camp és obligatori.");
        error++;
    ...
    if (!validBrand(w3M)) {
        showError(document.getElementById("w3M"), "errorw3M", "Aquest camp és obligatori.");
        error++;
    ...
    if (!validBrand(w4M)) {
        showError(document.getElementById("w4M"), "errorw4M", "Aquest camp és obligatori.");
        error++;
    ...
    if (!validDiameter(w1D)) {
        showError(document.getElementById("w1D"), "errorw1D", "El valor del diàmetre ha de ser entre 0.4 i 2");
        error++;
    ...
    if (!validDiameter(w2D)) {
        showError(document.getElementById("w2D"), "errorw2D", "El valor del diàmetre ha de ser entre 0.4 i 2");
        error++;
    ...
    if (!validDiameter(w3D)) {
        showError(document.getElementById("w3D"), "errorw3D", "El valor del diàmetre ha de ser entre 0.4 i 2");
        error++;
    ...
    if (!validDiameter(w4D)) {
        showError(document.getElementById("w4D"), "errorw4D", "El valor del diàmetre ha de ser entre 0.4 i 2");
        error++;
...
function validBrand(brand:string): boolean {
    var regex = /./gm;
